...
from aiofiles.os import path as aiopath, remove, makedirs, listdir, makedirs as aiomakedirs
...
class TaskConfig:
    def __init__(self):
        ...
        self.secondary_private_link = False
        ...
        self.secondary_drive_enabled = False
        self.secondary_drive_error = ""
        self.secondary_drive_source = ""
        self.upload_media_details = {}
    ...
    async def before_start(self):
        ...
        if not self.is_leech:
            ...
            tdlib_premium = TdlibManager.IS_AVAILABLE and TdlibManager.IS_PREMIUM_USER
            if self.bot_trans:
                self.user_transmission = False
                self.tdlib_transmission = False
                self.hybrid_leech = False
            if self.user_trans:
                ...
                self.tdlib_transmission = tdlib_premium
            if self.up_dest:
                ...
                if tdlib_premium:
                    try:
                        await TdlibManager.resolve_chat_id(self.up_dest)
                        self.tdlib_transmission = True
                    except Exception as e:
                        self.tdlib_transmission = False
                        LOGGER.warning(
                            f"TDLib can't use the destination chat {self.up_dest}: {e}"
                        )
                else:
                    self.tdlib_transmission = False
                ...
                self.user_transmission = False
                self.hybrid_leech = False
            elif self.tdlib_transmission and not self.is_super_chat:
                self.tdlib_transmission = False
            elif (
                not self.up_dest
                and tdlib_premium
                ...
                try:
                    await TdlibManager.resolve_chat_id(self.message.chat.id)
                    self.tdlib_transmission = True
                except Exception as e:
                    self.tdlib_transmission = False
                    LOGGER.warning(
                        f"TDLib can't use the current chat {self.message.chat.id}: {e}"
                    )
            if self.split_size:
            ...
            self.max_split_size = (
                ...
                if self.tdlib_transmission
                else 2097152000
            )
            if (
                self.tdlib_transmission
                and self.split_size == 2097152000
            ):
    ...
    async def resolve_secondary_gdrive_dest(self):
        self.stop_duplicate = (
            self.user_dict.get("STOP_DUPLICATE")
            or "STOP_DUPLICATE" not in self.user_dict
            and Config.STOP_DUPLICATE
        )
        dest = self.user_dict.get("GDRIVE_ID")
        if not dest and "GDRIVE_ID" not in self.user_dict:
            dest = Config.GDRIVE_ID
        if not dest:
            return

        self.secondary_drive_requested = True
        if dest == "gdl":
            self.secondary_drive_error = (
                "Automatic Google Drive backup can't use interactive gdl selection."
            )
            return
        if not is_gdrive_id(dest):
            self.secondary_drive_error = "Wrong Google Drive destination!"
            return
        ...
        token_path = self.get_token_path(dest)
        if token_path.startswith("tokens/"):
            self.secondary_private_link = True
        if not await aiopath.exists(token_path):
            self.secondary_drive_error = f"NO TOKEN! {token_path} not Exists!"
            return

        self.secondary_up_dest = dest
        self.secondary_drive_enabled = True
    ...
    async def proceed_extract(self, dl_path, gid):
        ...
        self.files_to_proceed = []
        self._fuse_mounts = getattr(self, '_fuse_mounts', [])
        ...
        # Collect archive list once from current download tree (avoid walking while mounts appear)
        archives = []
        for dirpath, _, files in await sync_to_async(walk, self.up_dir or self.dir, topdown=False):
            for file_ in files:
        ...
        if not archives:
            ...
            return dl_path
        # For single file archive, mount beside download dir
        if self.is_file and len(archives) == 1:
            f_path = archives[0]
            ...
            if isinstance(ret, str) and ospath.exists(ret):
                ...
                return ret
            return dl_path
        # Folder case: mount each archive to side mount inside self.dir, then merge view via symlinks
        view_dir = f"{self.dir}_extracted_view"
        await aiomakedirs(view_dir, exist_ok=True)
        for f_path in archives:
            if self.is_cancelled:
                return False
            self.proceed_count += 1
            self.subname = ospath.basename(f_path)
            # mount target is dl_path (folder) - SevenZ.extract will create _mounted subdir
            ret = await sevenz.extract(f_path, dl_path, pswd)
            if ret is False:
                LOGGER.error(f"Extract failed for {f_path}")
                continue
        ...
        for dirpath, _, files in await sync_to_async(walk, dl_path, topdown=False):
            for file_ in files:
                if is_archive(file_) or is_archive_split(file_) or is_first_archive_split(file_):
                    continue
                src = ospath.join(dirpath, file_)
                rel = ospath.relpath(src, dl_path)
                dst = ospath.join(view_dir, rel)
                await aiomakedirs(ospath.dirname(dst), exist_ok=True)
                if not await aiopath.exists(dst):
                    try:
                        await sync_to_async(os.symlink, src, dst)
                    except:
                        pass
        for mnt in self._fuse_mounts:
            for r, _, files in await sync_to_async(walk, mnt):
                rel = ospath.relpath(r, mnt)
                dst_base = ospath.join(view_dir, rel if rel != "." else "")
                await aiomakedirs(dst_base, exist_ok=True)
                for f in files:
                    src = ospath.join(r, f)
                    dst = ospath.join(dst_base, f)
                    if not await aiopath.exists(dst):
                        try:
                            await sync_to_async(os.symlink, src, dst)
                        except:
        ...
        if self.proceed_count == 0:
            LOGGER.info("No files able to extract!")
            return dl_path
        # If view has content return it, else fallback
        try:
            if await aiopath.exists(view_dir) and (await listdir(view_dir)):
                return view_dir
        except:
            pass
        return dl_path
    ...
    async def proceed_split(self, dl_path, gid):
        ...
        if self.files_to_proceed:
            ...
            for f_path, (f_size, file_) in self.files_to_proceed.items():
                ...
                # Check if f_path source directory is read-only (e.g. FUSE mount)
                parent_dir = ospath.dirname(f_path)
                test_touch = ospath.join(parent_dir, f".perm_{token_urlsafe(6)}")
                is_readonly_src = False
                try:
                    with open(test_touch, "wb") as tf:
                        tf.write(b"")
                    os.remove(test_touch)
                except Exception:
                    is_readonly_src = True

                out_dir = None
                if is_readonly_src:
                    # Place split parts in a dedicated writable directory inside self.dir
                    rel_dir = ospath.relpath(parent_dir, dl_path)
                    ...
                    out_dir = ospath.normpath(os.path.join(self.dir, f"{clean_name}_splits", rel_dir if rel_dir != "." else ""))
                ...
                if not self.as_doc and (await get_document_type(f_path))[0]:
                    async with task_dict_lock:
                        task_dict[self.mid] = FFmpegStatus(self, ffmpeg, gid, "Split")
                    res = await ffmpeg.split(f_path, file_, parts, split_size, out_dir=out_dir)
                else:
                    async with task_dict_lock:
                        task_dict[self.mid] = SplitStatus(self, f_path, f_size, gid)
                    res = await split_file(f_path, split_size, self, out_dir=out_dir)
                if self.is_cancelled:
                    return False
                if not is_readonly_src and (res or f_size >= self.max_split_size):
                    try:
                        await remove(f_path)
                    except:
                        self.is_cancelled = True

    async def proceed_split_streaming(self, dl_path, gid):
        # A: per-file split -> upload -> delete to keep peak low for FUSE mounts
        from bot.helper.mirror_leech_utils.telegram_uploader import TelegramUploader
        from bot.helper.mirror_leech_utils.status_utils.telegram_status import TelegramStatus
        from bot.helper.ext_utils.task_manager import check_running_tasks
        from bot import task_dict_lock, task_dict, upload_slots, intervals
        from bot.core.config_manager import Config
        from bot.core.telegram_manager import TgClient
        from bot.core.tdlib_manager import TdlibManager
        import os as _os
        # Mark streaming active so per-file tg.upload() -> on_upload_complete accumulates instead of finalizing
        ...
        for dirpath, _, files in await sync_to_async(walk, dl_path, topdown=False):
            for file_ in files:
                ...
                if f_size > self.split_size:
                    self.files_to_proceed[f_path] = [f_size, file_]
        # filter by zip picker selection if active
        ...
        if not _picker_small_only:
            total_files = len(self.files_to_proceed)
            LOGGER.info(f"Streaming split+upload: {self.name} files={total_files} splits_root={splits_root}")
            for idx, (f_path, (f_size, file_)) in enumerate(list(self.files_to_proceed.items()), 1):
                if self.is_cancelled:
                    self._streaming_active = False
                    return False
                self.proceed_count += 1
                ...
                self.subname = file_
                parts = -(-f_size // self.split_size)
                split_size = (f_size // parts) + (f_size % parts) if self.equal_splits else self.split_size
                parent_dir = ospath.dirname(f_path)
                rel_dir = ospath.relpath(parent_dir, dl_path)
                out_dir = ospath.normpath(os.path.join(splits_root, rel_dir if rel_dir != "." else ""))
                await aiomakedirs(out_dir, exist_ok=True)
                LOGGER.info(f"Split {idx}/{total_files}: {file_} size={f_size} -> {out_dir}")
                async with task_dict_lock:
                    task_dict[self.mid] = FFmpegStatus(self, ffmpeg, gid, "Split")
                self.progress = True
                if not self.as_doc and (await get_document_type(f_path))[0]:
                ...
                try:
                    await aiomakedirs(per_file_upload_dir, exist_ok=True)
                    for p in produced:
                        dst = ospath.join(per_file_upload_dir, ospath.basename(p))
                        try:
                            await sync_to_async(_os.symlink, p, dst)
                        except:
                            pass
                except:
                    per_file_upload_dir = out_dir
                if self.is_cancelled:
                    self._streaming_active = False
                    return False
                ...
                if add_to_queue:
                    async with task_dict_lock:
                        from bot.helper.mirror_leech_utils.status_utils.queue_status import QueueStatus as _QS1
                        task_dict[self.mid] = _QS1(self, gid, "Up")
                    await event.wait()
                    if self.is_cancelled:
                        self._streaming_active = False
                        return False
                # choose backend
                use_tdlib = (Config.TDLIB_USER_UPLOAD and TdlibManager.IS_AVAILABLE and self.tdlib_transmission and not self.hybrid_leech and self.size >= Config.TDLIB_USER_UPLOAD_MIN_SIZE)
                from bot.helper.mirror_leech_utils.tdlib_uploader import TdlibTelegramUploader as _Tdlib1
                upload_base = per_file_upload_dir
                if use_tdlib:
                    tg = _Tdlib1(self, upload_base)
                else:
                    tg = TelegramUploader(self, upload_base)
                async with task_dict_lock:
                ...
                try:
                    ...
                    LOGGER.error(f"Streaming upload failed for {file_}: {e}")
                    # keep split parts for retry/manual, don't delete, continue to next
                    continue
                if self.is_cancelled:
                    self._streaming_active = False
                    return False
                # On success, delete the split parts for this file to reclaim space
                for p in produced:
                    try:
                        await remove(p)
                    except:
                        pass
                try:
                    await sync_to_async(_os.rmdir, per_file_upload_dir) if await aiopath.exists(per_file_upload_dir) else None
                except:
                    pass
                LOGGER.info(f"Finished streaming file {idx}/{total_files}: {file_} uploaded and cleaned")        # After streaming all large files, handle remaining small files (not split) by uploading from mount directly in one final batch
        remaining_small = []
        sel_rels = getattr(self, '_zip_selected_rels', None)
        for dirpath, _, files in await sync_to_async(walk, dl_path, topdown=False):
            for file_ in files:
                f_path = ospath.join(dirpath, file_)
                if f_path in self.files_to_proceed:
                    continue
                if sel_rels:
                    rel = ospath.relpath(f_path, dl_path)
                    if rel not in sel_rels:
                        LOGGER.info(f"Zip picker skip small: {rel}")
                        continue
                try:
                    f_size = await get_path_size(f_path)
                except:
        ...
        if remaining_small:
            ...
            for r_idx, f_path in enumerate(remaining_small, 1):
                ...
                try:
                    ...
                    per_small_dir = ospath.dirname(f_path)
                # one-by-one queue slot
                add_to_queue, event = await check_running_tasks(self, "up")
                if add_to_queue:
                    async with task_dict_lock:
                        from bot.helper.mirror_leech_utils.status_utils.queue_status import QueueStatus as _QS2
                        task_dict[self.mid] = _QS2(self, gid, "Up")
                    await event.wait()
                    if self.is_cancelled:
                        self._streaming_active = False
                        return False
                use_tdlib = (Config.TDLIB_USER_UPLOAD and TdlibManager.IS_AVAILABLE and self.tdlib_transmission and not self.hybrid_leech and f_size >= Config.TDLIB_USER_UPLOAD_MIN_SIZE)
                from bot.helper.mirror_leech_utils.tdlib_uploader import TdlibTelegramUploader as _Tdlib2
                if use_tdlib:
                    tg = _Tdlib2(self, per_small_dir)
                else:
                ...
                try:
                    ...
                    LOGGER.error(f"Small file upload failed for {f_path}: {e}")
                    # keep going to next small file
                    try:
                        from aioshutil import rmtree as _rmtree
        ...
        # Return splits_root as the logical upload dir (now mostly empty, but keeps path for cleanup)
        return splits_root
