    ...
    while i < total:
        ...
        if part in arg_base:
            ...
            if part in bool_arg_set:
                arg_base[part] = True
            elif part in single_val_set:
                if i + 1 < total and items[i + 1] not in arg_base:
                    arg_base[part] = items[i + 1]
                    consumed[i + 1] = True
                    i += 1
            else:
                sub_list = []
                for j in range(i + 1, total):
                    if items[j] in arg_base:
                        if part == "-c" and items[j] == "-c":
                            ...
                            consumed[j] = True
                            continue
                        if items[j] in bool_arg_set and not sub_list:
                            break
                        if not sub_list:
                            break
                    ...
                    if part == "-ff":
                        if not value.strip().startswith("["):
                            ...
                            try:
                                ...
                                arg_base[part].add(tuple(parsed) if isinstance(parsed, (list, tuple)) else parsed)
                            except Exception:
                                pass
                    else:
                        arg_base[part] = value
        ...
        i += 1
    if "link" in arg_base:
        link_parts = [items[idx] for idx in range(total) if not consumed[idx]]
        if link_parts:
