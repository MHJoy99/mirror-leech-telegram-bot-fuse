...
async def load_settings():
    ...
    config_is_valid = bool(config_file) and required_keys.issubset(config_file)

    old_config = await database.db.settings.deployConfig.find_one(
        {"_id": BOT_ID}, {"_id": 0}
    )
    if old_config is None and config_is_valid:
        await database.db.settings.deployConfig.replace_one(
            {"_id": BOT_ID}, config_file, upsert=True
        )
    elif old_config and config_is_valid and old_config != config_file:
        LOGGER.info("Replacing existing deploy config in Database")
        await database.db.settings.deployConfig.replace_one(
            {"_id": BOT_ID}, config_file, upsert=True
        )
    elif old_config and not config_is_valid:
        LOGGER.warning(
            "Ignoring invalid local config while keeping the stored deploy config"
        )
    else:
        config_dict = await database.db.settings.config.find_one(
            {"_id": BOT_ID}, {"_id": 0}
        )
        if config_dict and required_keys.issubset(config_dict):
            Config.load_dict(config_dict)
        elif config_dict:
            LOGGER.warning(
                "Ignoring incomplete config from database because required keys are missing"
            )
...
async def update_variables():
    Config.TG_SPLIT_UPLOAD_CONCURRENCY = max(
        1, min(int(Config.TG_SPLIT_UPLOAD_CONCURRENCY), 4)
    )
    Config.TG_FILE_UPLOAD_CONCURRENCY = max(
        1, min(int(Config.TG_FILE_UPLOAD_CONCURRENCY), 16)
    )
    Config.TG_UPLOAD_WORKERS = max(1, min(int(Config.TG_UPLOAD_WORKERS), 16))
    upload_slots.set_limit(Config.TG_FILE_UPLOAD_CONCURRENCY)
    LOGGER.info(
        "Telegram upload tuning: "
        f"workers/file={Config.TG_UPLOAD_WORKERS}, "
        f"file_parallel={Config.TG_FILE_UPLOAD_CONCURRENCY}, "
        f"split_parallel={Config.TG_SPLIT_UPLOAD_CONCURRENCY}"
    )
    if Config.TDLIB_USER_UPLOAD:
        db_paths = getattr(Config, "TDLIB_USER_DB_PATHS", []) or [TdlibManager._db_path()]
        LOGGER.info(
            "TDLib upload tuning: "
            f"enabled={Config.TDLIB_USER_UPLOAD} | "
            f"min_size={Config.TDLIB_USER_UPLOAD_MIN_SIZE} | "
            f"dbs={db_paths} | ready={TdlibManager.IS_AVAILABLE}"
        )
